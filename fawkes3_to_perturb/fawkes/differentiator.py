...
class FawkesMaskGeneration:
    ...
    def __init__(self, sess, bottleneck_model_ls, mimic_img=MIMIC_IMG,
        ...
        self.modifier = tf.Variable(np.zeros(self.input_shape, dtype='float32'))

        # target image in tanh space
        if self.MIMIC_IMG:
            self.timg_tanh = tf.Variable(np.zeros(self.input_shape), dtype='float32')
        else:
            self.bottleneck_t_raw = tf.Variable(np.zeros(self.bottleneck_shape), dtype='float32')
        # source image in tanh space
        self.simg_tanh = tf.Variable(np.zeros(self.input_shape), dtype='float32')

        self.const = tf.Variable(np.ones(batch_size), dtype='float32')
        self.mask = tf.Variable(np.ones((batch_size),  dtype='bool'))
